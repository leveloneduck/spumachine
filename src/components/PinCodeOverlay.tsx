...
const PinCodeOverlay: React.FC = () => {
  ...
  const { verifyPin, isLoading, error } = usePinAuth();
  ...
  return (
    ...
    >
      <div className="w-full max-w-md mx-auto p-8">
            ...
            <InputOTPGroup>
              {Array.from({ length: 6 }).map((_, index) => (
                <InputOTPSlot
                  key={index}
                  index={index}
                  className="w-14 h-14 text-2xl border-2 border-border/50 
        ...
        {/* Error Message */}
        {error && (
          <div className="mb-6 p-3 rounded-md bg-destructive/20 border border-destructive/50 text-center">
